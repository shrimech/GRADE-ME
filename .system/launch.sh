SALAH="welcome with salah to the simulation"
...
while [ -d /proc/$PID ]; do
    for i in "${spin[@]}"; do
        echo -ne "$LINE_UP$WHITE$i$RED Checking server availability\n"
        #for i in {1..32}; do
            #printf "\b"
        #done
        sleep 0.5
...
if [ "$1" != "gradejustinstall" ]; then
    ...
    if ! ping -c 1 google.com >/dev/null 2>&1; then
        ...
        # clear
        printf "$GREEN$BOLD"
	echo -ne "($SALAH)\n✔$RESET You have the last version$GREEN$BOLD version:($version)\n\n"
